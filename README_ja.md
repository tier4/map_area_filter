...
### パラメータ設定（Global）
物体と点群それぞれに対して、フィルタ処理を適用するか否かを一括で設定できます。
...
#### 2. 領域設定
Lanelet上の点列によってポリゴンを設定します。内外判定仕様を加味した上で、適切な形状に設定してください。
...
物体種別がKeyに登録されていない場合やValueが`remove`に設定されていない場合は、そのエリアでの物体の除去は行われません。
